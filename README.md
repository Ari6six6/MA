...
| **The almanac** | At the end of every run, the librarian checks this run's expected-vs-actual outcomes; where they diverge, a bounded pass forms a real hypothesis for WHY (researching it with `web_search`/read-only `http_request` when useful) and banks it to a store shared across **every project**, not just this one — an index in every system prompt, the full writeup via `load_almanac`. | **on** |
