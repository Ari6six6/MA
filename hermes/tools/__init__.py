...
def build_registry(project, cfg, confirm_fn) -> ToolRegistry:
    ...
    # when the operator opts in — standingly via config, or for one `improve`
    # sitting via a per-call config copy (hermes/cli.py). Gated far tighter
    # than project file tools — see hermes/tools/self_build.py's PROTECTED
    # denylist.
